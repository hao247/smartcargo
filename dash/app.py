...
app.layout = html.Div(children=[
    html.Div([
        ...
        html.Div([
            html.Label(['Departure port'],style={'color': 'orange', 'font-size': '150%'}),
            dcc.Dropdown(
                id='port_start',
                options=ports,
        ...
        html.Div([
            html.Label(['Destination port'],style={'color': 'red', 'font-size': '150%'}),
            dcc.Dropdown(
                id='port_end',
                options=ports,
    ...
    html.Div([
        ...
        dcc.Graph(
            id='map',
            figure=p.generate_map(port_list, 'null', 'null'),
        ),
...
def show_departure_port_counts(port_start):
    outbound_query = "select * from trips_final_1 where departure = '{}'".format(port_start)
    inbound_query = "select * from trips_final_1 where arrival = '{}'".format(port_start)
    outbound = read_pd_df(outbound_query)
    inbound = read_pd_df(inbound_query)
    col1 = outbound\
            .time_start\
            .dt.week\
            if not outbound.empty else outbound.time_start
    col2 = inbound\
            .time_start\
            .dt.week\
            if not inbound.empty else inbound.time_end
    return p.generate_stacked_histogram(col1, [], port_start, 'Week', 'Count')


@app.callback(Output('destination_port_traffic', 'figure'),
        [Input('port_end', 'value')])
def show_arrival_port_counts(port_end):
    outbound_query = "select * from trips_final_1 where departure = '{}'".format(port_end)
...
@app.callback(Output('heat_map_start', 'figure'),
        [Input('port_start', 'value')])
...
@app.callback(Output('heat_map_end', 'figure'),
        [Input('port_end', 'value')])
