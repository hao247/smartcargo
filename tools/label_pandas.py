...
def label_ports():
    ...
    for index, port in ports.iterrows():
        ...
        scores = [0, 4, 2]
        score_cols = ['lat_start_score', 'lon_start_score',
                      'lat_end_score', 'lon_end_score']
        coord_cols = ['lat_start', 'lon_start', 'lat_end', 'lon_end']
        bins = [lat_bin, lon_bin, lat_bin, lon_bin]
        for i in len(score_cols):
            trips[score_cols[i]] = pd.cut(trips[coord_cols[i]], bins[i],
                                          labels=scores).astype(int)
        ...
        trips['departure'] = pd.cut(trips['start_score'], total_bin,
                                    labels=total_scores).astype(str)
        trips['arrival'] = pd.cut(trips['end_score'], total_bin,
                                  labels=total_scores).astype(str)
    ...
    for index, row in trips.iterrows():
        command = 'insert into trip_labeled
                   (tripid, time_start, time_end, departure, arrival, duration)
                   values (\'{}\', \'{}\', \'{}\', \'{}\', \'{}\', {})'\
