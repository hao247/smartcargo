...
def group_trips(df):
    ...
    df = df.filter(df.SOG > sog_thres).withColumn(
        "PrevTime", f.lag(df.BaseDateTime).over(window)
    )

    df = df.withColumn(
        "DiffTime", f.unix_timestamp(df.BaseDateTime) - f.unix_timestamp(df.PrevTime)
    )

    df = df.withColumn(
        "TripID",
        f.when(
            (df.DiffTime > diff_time_thres) | (f.isnull(df.DiffTime)),
...
def gen_trip_table(df):
    ...
    df = df.select(
        ...
        *[
            f.first(f.col(col_name)).over(window_desc).alias(col_name + "_END")
    ...
    df = df.select(
        *[c for c in df.columns],
        *[
            f.lead(f.col(col_name)).over(window).alias("LAST_" + col_name)
            for col_name in lead_labels
        ]
    )
    ...
    df = (
        df.withColumn(
            "BaseDateTime_END",
            f.when(
                ~(f.isnull(df.LAST_PREV_BaseDateTime)), df.LAST_PREV_BaseDateTime
            ).otherwise(df.BaseDateTime_END),
        )
        .withColumn(
            "LAT_END",
            f.when(~(f.isnull(df.LAST_PREV_LAT)), df.LAST_PREV_LAT).otherwise(
                df.LAT_END
            ),
        )
        .withColumn(
            "LON_END",
            f.when(~(f.isnull(df.LAST_PREV_LON)), df.LAST_PREV_LON).otherwise(
                df.LON_END
            ),
        )
        .select(
    ...
    df = df.filter(
        f.unix_timestamp(df.TIME_END) - f.unix_timestamp(df.TIME_START)
        > time_diff_thres
    )
