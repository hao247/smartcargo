...
def bin_location(df, lat, lon, rang, input_cols):
    ...
    for input_col in input_cols:
        ...
        if input_col[:3] == "LAT":
            ...
            splits = lon_splits
        bucketizer = Bucketizer(splits=splits, inputCol=input_col,
...
def label_ports(df, rang):
    ...
    for index, port in ports.iterrows():
        ...
        df = df.withColumn('PORT_START', f.when(df.SCORE_START == 1.0, port_name)
                           .otherwise(df.PORT_START))
        df = df.withColumn('PORT_END', f.when(df.SCORE_END == 1.0, port_name)
                           .otherwise(df.PORT_END))
    ...
    for index, port in ports.iterrows():
        ...
        for input_col in input_cols:
            ...
            if input_col[:3] == "LAT":
                ...
                splits = lon_splits
            bucketizer = Bucketizer(splits=splits, inputCol=input_coal,
                                    outputCol = output_col)
            df = bucketizer.transform(df)
        
        df = df.withColumn('SCORE', df.LAT_SCORE*df.LON_SCORE)
        df = df.withColumn('PORT', f.when(df.SCORE ==1.0, port_name)
                           .otherwise(df.PORT))
        df = df.drop(*scores)
