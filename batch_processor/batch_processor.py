...
class BatchProcessor:
    ...
    def read_from_s3(self):
        ...
        """
        s3file = "s3a://{}/{}/{}".format(
            self.s3_conf["bucket"], self.s3_conf["folder"], self.s3_conf["file"]
        )
    ...
    def save_to_psql(self, df, table_name, mode):
        ...
        df.write.format("jdbc").option("url", self.credent["psql"]["url"]).option(
            "driver", self.credent["psql"]["driver"]
        ).option("user", self.credent["psql"]["user"]).option(
    ...
    def spark_transform(self):
        ...
        """
        self.ship_info_df = self.df.select(
            "MMSI", "VesselName", "IMO", "Length", "Width", "Draft", "Cargo"
        ).distinct()
        ...
        self.trip_log_df = self.df_with_label.select(
